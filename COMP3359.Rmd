...
In this section, we will read the data into our IDE (i.e. in our case, to RStudio). However, we have done some slight modification towards the previously cluttered data using jupyter notebook and managed to obtain the V1 column which is used for storing the movie id which will be our input data in this case.
...
Referring to the definition of data engineering, 
```{r}
data_summary <- data %>% 
  group_by(rating)  %>% 
  count(rating) %>% 
...
```{r}
save_model_hdf5(model, "model_netflix.h5")
```


```{r}
model_test <- load_model_hdf5("model_netflix.h5")
```
...
pred2 <- model_test %>% predict(inputs)
...
```{r}
combined_light %>% 
      filter(combined_light$date>=format("1/1/2000"))
```
```{r}
arrange(combined_light, year_released) %>% 
  head(1) %>% 
  select(year_released) %>% 
  pull()
head_light <- combined_light %>% 
  arrange(combined_light$year_released) %>%
  mutate(date = as.POSIXct(date)) %>% head(1) 
format(head_light$date, "%Y")
```
