...
```{r}
combined_light <- inner_join(data, group, by = "customer_id")
combined_light <- inner_join(combined_light, top_users, by = "movie_id")
...
training <- combined_light[index, ]
validation <- combined_light[-index,]

x_train <- training %>% select(c(customer_id_dense, movie_id_dense))
row.names(x_train) <- NULL
x_train <- x_train %>% as.matrix()

y_train <- training %>% pull(rating)

x_val <- validation %>% select(c(customer_id_dense, movie_id_dense)) %>% as.matrix()
row.names(x_val) <- NULL
x_val <- x_val %>% as.matrix()

y_val <- validation %>% select(rating) %>% as.matrix()
```
